...
class MainWindow(QMainWindow):
	...
	def initUI(self):
		...
		#self.restoreSetting("splitterSizes", splitter)

	def restoreSetting(self, setting, name):
		settings = QSettings()
		name.restoreState(settings.value(setting).toByteArray())

	def setSetting(self, setting, name):
		settings = QSettings()
		settings.setValue('{}, {}.saveState()'.format(setting, name))
	...
	def tabTextSearchUI(self):
		...
		splitter2.setSizes([self.initWidth/2, self.initWidth/2])
...
class TextFinder(QWidget):
	...
	def applyHighlight(self):
		...
		captureDoc = self.textCaptures.document()

		cursor = QTextCursor(document)
		cursor2 = QTextCursor(matchDoc)
		cursor3 = QTextCursor(captureDoc)
		hlCursor = QTextCursor(self.textEdit.textCursor())
		hlCursor2 = QTextCursor(self.textCaptures.textCursor())
		hlCursor3 = QTextCursor(self.textReplace.textCursor())
		...
		match_list = deque()
		rx = QRegularExpression(self.inputData)
		...
		for i in pat.finditer(document.toPlainText()):
			...
			hlCursor.setPosition(i.start(0), QTextCursor.MoveAnchor)
			hlCursor.setPosition(i.end(0), QTextCursor.KeepAnchor)
			extra = QTextEdit.ExtraSelection()
		...
		for group in range(pat.groups):
			...
			for j, match in enumerate(match_list):
				self.textCaptures.appendPlainText('{}. {}'.format(j, match.group(group)))
	...
	def applyReplace(self):
		...
		# Avoid errors with unmatched parens, etc.
		try:	
	...
	def applySplit(self):
		...
		try:
			...
			try:
				re.sub(self.lineEdit, self.replaceLineEdit2, str(rp))
			except TypeError:
				pass
		except ValueError:
	...
	def loadTextFile(self):
		inputFile = QFile(self.searchOpenFile(), self)
